...
class OpenAIRealtimeConnection:
    ...
    async def send_session_update(self, config: Optional[dict] = None):
        ...
        default_config = {
            ...
            "tools": get_tool_definitions()  # Function Call定義を追加
        }
...
async def handle_function_call(message: dict, openai_conn: OpenAIRealtimeConnection, session_id: str):
    """
    ...
    try:
        ...
        # Function Call結果を送信した後、新しいレスポンスを生成
        create_response = {
            ...
            try:
                while openai_conn.is_connected:
                    ...
                    if "text" in data:
                        ...
                        # 音声データが含まれているか確認
                        if event_type == "input_audio_buffer.append" and "audio" in message:
                            # 音声データがある場合はカウント
                            session_info.increment_audio_sent()
                            logger.debug(f"音声データ転送: {len(message['audio'])} chars (base64)")
                ...
                if "disconnect" in str(e).lower():
                    logger.info(f"クライアント接続が閉じられました (session: {session_id})")
                else:
                ...
                while openai_conn.is_connected:
                    ...
                    if message:
                        ...
                        try:
                            ...
                            # クライアントが切断済み
                            logger.info(f"クライアントが切断済みのためメッセージ送信をスキップ (session: {session_id})")
                            break
