...
class OpenAIConfig(BaseModel):
    ...
    model: str = Field(
        default="gpt-4o-realtime-preview-2024-12-17", description="使用するモデル"
    )
    api_url: str = Field(
...
class SessionConfig(BaseModel):
    ...
    modalities: list[str] = Field(
        default=["text", "audio"], description="モダリティ（テキスト、音声）"
    )
    ...
    input_audio_format: str = Field(default="pcm16", description="入力音声フォーマット")
    output_audio_format: str = Field(
        default="pcm16", description="出力音声フォーマット"
    )
    input_audio_transcription_model: str = Field(
        default="whisper-1", description="文字起こしモデル"
    )
    turn_detection_type: str = Field(
        default="server_vad", description="ターン検出タイプ"
    )
    turn_detection_threshold: float = Field(default=0.5, description="音声検出閾値")
    turn_detection_prefix_padding_ms: int = Field(
        default=300, description="音声開始前のパディング（ミリ秒）"
    )
    turn_detection_silence_duration_ms: int = Field(
        default=500, description="無音と判定する時間（ミリ秒）"
    )
    temperature: float = Field(default=0.8, description="応答の温度（ランダム性）")
    max_response_output_tokens: int = Field(
        default=4096, description="最大レスポンストークン数"
    )
...
class Config:
    ...
    def __init__(self):
        ...
        self.server = ServerConfig(log_level=os.getenv("LOG_LEVEL", "INFO"))
